...
-- Remove trigger/função de segurança antiga que pode depender de
-- current_user_role() e impedir a troca do tipo de retorno.
...
-- can_view_student() também depende de current_user_role().
drop function if exists public.can_view_student(uuid);

-- Agora current_user_role() pode ser removida com segurança,
-- independentemente de a versão antiga retornar app_role ou text.
